...
export const GET: RequestHandler = async ({ locals }) => {
  ...
  const db = getDb();
  const row = db.prepare('SELECT wikiPath FROM wiki_pages WHERE userId = ? AND pageType = ?').get(userId, 'index') as { wikiPath: string } | undefined;
  const wikiPath = row?.wikiPath ?? 'wiki/index.md';
  const vaultRoot = getUserVaultPaths(userId).root;

  try {
    return json({ markdown: readVaultTextFile(vaultRoot, wikiPath), wikiPath });
  } catch {
    return json({ markdown: '# Wiki Index\n', wikiPath });
  }
};
